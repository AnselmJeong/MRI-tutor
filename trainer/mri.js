...
export async function createMRI({data,onLocation}){
 let nv,token=0,busy=false,space=null,selected=3,plane='multi',concealed=false,lastPoint=[0,0,0];
 const cache=new Map();
 ...
 const setPoint=(mm)=>{nv.scene.crosshairPos=nv.mm2frac(mm);nv.drawScene();nv.createOnLocationChange();};
 function colors(){
 ...
 async function select(id,{move=true}={}){
  ...
  busy=true;$('mri-loading').hidden=false;$('mri-loading').textContent='MRI와 구조 연결 중…';
  try{
   ...
   setPoint(move?r.anchor:lastPoint);return true;
  }catch(error){if(mine===token){busy=false;$('mri-loading').hidden=false;$('mri-loading').textContent='구조를 불러오지 못했습니다. 다른 구조를 선택하거나 새로고침하세요.';}console.error(error);return false;}
 }
