...
import {subjectCoreTopics} from '../trainer/viewer/space-registry.js';
...
try{
 await page.goto('http://127.0.0.1:8091/');await page.locator('#cases-mode').click();await ready();await page.locator('[data-case-mode=explore]').click();
 ...
 const unavailable=async()=>{expect(await page.evaluate(()=>window.mriSubjectQA.snapshot().visibleStructureIds)).toEqual([]);expect(await page.evaluate(()=>window.mriSubjectQA.snapshot().atlasVisible)).toBe(false);};
 const checked=[];
 for(const t of expandedTopics){
  ...
  for(const side of t.midline?['midline']:['left','right']){
   ...
   if(!subjectCoreTopics.includes(t.id)){await expect(page.locator('[data-hint=three]')).toHaveCount(0);await unavailable();}else await expect(page.locator('[data-hint=three]')).toHaveCount(1);
 ...
 await page.locator('[data-case-mode=compare]').click();await page.locator('input[value=indeterminate]').check();await page.locator('#reading-note').fill('교뇌와 제4뇌실의 관계는 확인했지만 내부 미세 핵의 경계는 이 대비에서 확정할 수 없다.');await page.locator('#reading-submit').click();await unavailable();
 expect((await state()).task.submitted.contractId).toContain('brainstem');await page.reload();await page.locator('#cases-mode').click();await ready();await expect(page.locator('#case-side')).toHaveValue('midline');await expect(page.locator('.reading-feedback')).toBeVisible();
 await unavailable();await page.locator('[data-case-mode=explore]').click();await unavailable();
