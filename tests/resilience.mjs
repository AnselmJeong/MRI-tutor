...
try{
 const {page,context}=await make();await page.goto('http://127.0.0.1:8091/');await ready(page);
 ...
 await expect(page.locator('#reading-submit')).toBeDisabled();await expect(page.locator('#case-mri')).toHaveCSS('visibility','hidden');await expect(page.locator('#case-loading')).toContainText('다시 불러오기');
 ...
 await page.locator('#quiz-open').click();await page.locator('#quiz-start').click();await page.locator('.quiz-options button').first().click();await expect(page.locator('#quiz-next')).toBeEnabled();await page.locator('[data-close=quiz]').click();
 await page.locator('#cases-mode').click();await expect(page.locator('#isolate')).toBeEnabled();logs.push('Legacy atlas localization, 3D quiz, foundation quiz and return restore controls');
 await context.close();
 const second=await make();await second.page.addInitScript(()=>{Storage.prototype.getItem=function(){throw new DOMException('blocked','SecurityError')};Storage.prototype.setItem=function(){throw new DOMException('blocked','QuotaExceededError')};});await second.page.goto('http://127.0.0.1:8091/');await ready(second.page);await second.page.locator('input[value=indeterminate]').check();await second.page.locator('#reading-note').fill('저장소를 사용할 수 없어도 관찰과 현재 세션의 첫 응답은 유지되어야 한다.');await second.page.locator('#reading-submit').click();await expect(second.page.locator('.reading-feedback')).toBeVisible();await second.page.locator('#case-history').click();await expect(second.page.locator('#case-dialog')).toContainText('1개 첫 응답');logs.push('Blocked localStorage still permits learning, live history and export');await second.context.close();
